...
foreach ($file in $files) {
    ...
    try {
        ...
        while ($r -le $rows) {
            ...
            if (-not [string]::IsNullOrWhiteSpace($parentVal)) {
                ...
                # Mantener registro de locations para las que ya hemos añadido el registro padre (evita duplicados)
                $parentAddedLocations = @{}
                ...
                while ($rChild -le $rows) {
                    ...
                    for ($c = 3; $c -le $cols; $c++) {
                        ...
                        if ($include) {
                            ...
                            # Si aún no hemos añadido el registro padre para esta Location, añadirlo primero
                            if (-not $parentAddedLocations.ContainsKey($locName)) {
                                $parentObj = [PSCustomObject]@{
                                    ...
                                    'Parent Asset' = ''    # vacío para indicar que es el padre
                                    Area = ''
                                    Barcode = ''
                                    Category = ''
                                    'Primary User' = ''
                                ...
                                $results += $parentObj
                                $parentAddedLocations[$locName] = $true
                            }

                            # Crear objeto hijo con todas las propiedades de la plantilla
                            $obj = [PSCustomObject]@{
                                ID = ''
                                Name = [string]$nameVal
                                Description = ''
    ...
    if ($results.Count -eq 0) {
        ...
        # Ordenar por Location Name (A-Z), luego por si es padre (padres primero) y luego por Name para determinismo
        $sorted = $results | Sort-Object -Property @{Expression={$_. 'Location Name'}}, @{Expression={ if ([string]::IsNullOrWhiteSpace($_.'Parent Asset')) { 0 } else { 1 } }}, 'Name'
        ...
        Write-Host "CSV generado (ordenado por Location Name, padres antes que hijos): $outPath"
    }
}
