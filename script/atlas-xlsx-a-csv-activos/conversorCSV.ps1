...
.DESCRIPCION
    ...
    - Salida: <archivo>-CSV-OK-<fechaCreacion>.csv (UTF-8 BOM) con plantilla fija; solo Name, Parent Asset y Location Name rellenados.
#>
...
try {
    ...
    # Rutas de salida y marca temporal basada en CreationTime del Excel
    $excelFolder = Split-Path -Path $ExcelFile -Parent
    $excelBase = [IO.Path]::GetFileNameWithoutExtension($ExcelFile)

    # --- Aquí se obtiene la fecha de creación y se formatea ---
    $fileInfo = Get-Item -Path $ExcelFile
    $creationDatePart = $fileInfo.CreationTime.ToString("yyyyMMdd_HHmmss")
    # ---------------------------------------------------------

    # Construir nombres de salida incluyendo la fecha de creación
    $outCsv = Join-Path $excelFolder ("{0}-CSV-OK-{1}.csv" -f $excelBase, $creationDatePart)
    $logFile = Join-Path $excelFolder ("{0}-CSV-OK-{1}.log" -f $excelBase, $creationDatePart)

    if (Test-Path $logFile) { Remove-Item $logFile -Force }
    Write-Log -Message "Inicio de conversión. Plantilla embebida; Excel: $ExcelFile; FechaCreacion: $creationDatePart" -LogPath $logFile
